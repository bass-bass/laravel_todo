  ...
  @component('layouts.sidebar-left',['user'=>Auth::user()->load('articles','follows','followers')])
    @endcomponent
      ...
      <div class="col-lg-8 vstack gap-4">
          ...
          <div class="card">
              <!-- プロフィール 開始 -->
              @component('layouts.page', ['user' => $user, 'page'=>'follow'])
              @endcomponent
              <!-- プロフィール 終了 -->
          </div>
          <!-- Card END -->
          ...
          <div class="card-body">
            ...
            <div class="d-md-flex align-items-center">
                ...
                <a href="{{ route('user.home', ['user_id' => $follow->user_id]) }}"class="text-reset">
                  @if(is_null($follow->image))
                  <img class="avatar-img rounded-circle" src="{{asset('images/noimage.png')}}" alt="avatar">
                 @else
                  <img class="avatar-img rounded-circle" src="data:image/png;base64,{{ $follow->image }}" alt="avatar">
                 @endif
                  <small>{{ '@' . $follow->user_id }}</small></a>
                </div>
              <!-- Info -->
              <div class="w-100">
                <div class="d-sm-flex align-items-start">
                    <h6 class="mb-0">{{ $follow->name }}</a></h6>
                </div>
              </div>
            </div>
            </div>
